...
⚠️ **Чем это оплачено и что из этого закрыто.** Шаги на широком экране —
растр, и из этого следовало:
- на телефоне они не читались — **закрыто:** ниже 700 px картинка обрезается
  до шапки, а шаги идут живым текстом;
- ссылки внутри шагов перестали быть кликабельными — **закрыто:** на широком
  экране поверх подчёркнутых слов лежат прозрачные `<a>`, на узком работает
  живой текст;
- в поиск и в скринридер на широком экране они попадают только через `alt`;
- любая правка формулировки требует пересборки картинки **и** правки живого
  списка. `build.py` сверяет их и падает, если они разошлись.
