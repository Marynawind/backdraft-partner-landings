...
STEP_CAP, STEP_LEAD, STEP_GAP = 34, 52, 19   # высота прописной, интерлиньяж, поля пункта
NUM_CAP = 36
STRIP_CAP, STRIP_LEAD = 19, 28               # полоса — сноска, поэтому мельче шагов
ICON_H = 62
...
# сюда НЕ входит — при ширине 320 px она оказалась бы высотой 4 px. На узком
# экране её несёт живой текст, поэтому и дублирования не возникает.
HEAD_BOTTOM = 420
# Правый край шапки: заголовок кончается на x≈1165, рендер начинается с x≈1196.
# На узком экране картинка обрезается и по горизонтали, иначе от рендера
# остаётся торчащий обрубок сбоку от заголовка.
HEAD_RIGHT = 1180
...
# Прописная 24 — потолок: при 25 строка выходит за правый край заголовка
# (1187 против 1165) и за стёртую область, а переносить её в две строки некуда:
# до декоративной линейки под ней остаётся 75 px.
...
def compose(poster: Image.Image, strip_src: Image.Image) -> Image.Image:
    ...
    POSTER_MAP.write_text(json.dumps({
        ...
        "head": {"w": poster.width, "h": HEAD_BOTTOM, "right": HEAD_RIGHT},
        "steps": STEPS,
