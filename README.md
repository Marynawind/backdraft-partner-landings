...
Почти никакого JS — страница целиком статична и рендерится сразу; анимация
иллюстраций сделана на CSS и полностью отключается при
`prefers-reduced-motion: reduce`. Единственное исключение — десяток строк
в конце страницы Meprolight: правило «серийник ИЛИ чек», которого в HTML нет.
Скрипт только выставляет полям текст ошибки, дальше форму не пускает сам
браузер; если скрипт вырезать, форма остаётся рабочей.
